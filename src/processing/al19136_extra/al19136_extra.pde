...
void setup() {
  ...
  // Fill the tables
  radiusPhase = new float[SINCOS_LENGTH];
  sinLUT=new float[SINCOS_LENGTH];
  cosLUT=new float[SINCOS_LENGTH];
  for (int i = 0; i < SINCOS_LENGTH; i++) {
    radiusPhase[i] = random(PI * 2);
  ...
  for (int i=0; i<num; i++) {
    ...
    // get colors
    prob = random(100);
    if(prob<30) style[i*2]= defaultColor[i * 2] = colorBlended(random(1), 255,0,100, 255,0,0, 210);
    else if(prob<70) style[i*2]= defaultColor[i * 2] = colorBlended(random(1), 0,153,255, 170,225,255, 210);
    else if(prob<90) style[i*2]= defaultColor[i * 2] = colorBlended(random(1), 200,255,0, 150,255,0, 210);
    else style[i*2] = defaultColor[i * 2] =color(255,255,255, 220);

    if(prob<50) style[i*2]= defaultColor[i * 2] = colorBlended(random(1), 200,255,0, 50,120,0, 210);
    else if(prob<90) style[i*2]= defaultColor[i * 2] = colorBlended(random(1), 255,100,0, 255,255,0, 210);
    else style[i*2] = defaultColor[i * 2] =color(255,255,255, 220);

    style[i*2+1] = defaultColor[i * 2 + 1] = (int)(random(100))%3;
  }
}
...
void draw() {
  ...
  background(0);
  int in = arduino.analogRead(input0);
  text(in, 10, 20);
  fill(255,255,255);
  float radpt = float(in) / 1023 * PI;
  ...
  for (int i = 0; i < num; i++) {
    ...
    pt[index + 3] += 50 * (sin(radiusPhase[i] + (PI * 2) / 360 * radpt) - sin(radiusPhase[i]));
    radiusPhase[i] += (PI * 2) / 360 * radpt;
    int r = int(float((defaultColor[i * 2] & 0xff000000) >>> 24) * radpt / PI);
    int g = int(float((defaultColor[i * 2] & 0x00ff0000) >>> 16) * radpt / PI);
